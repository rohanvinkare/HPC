...
// [Previous PGMImage class remains unchanged...]
...
// Parallel reduction using local arrays
#pragma omp parallel
    {
...
#pragma omp parallel
    {
        double local_minDistance = numeric_limits<double>::max();
        int local_closestIndex = -1;

#pragma omp for nowait
        for (int i = 0; i < numTraining; i++)
        {
            double distance = computeDistance(queryFeatures, trainingFeatures[i]);
            if (distance < local_minDistance)
            {
                local_minDistance = distance;
                local_closestIndex = i;
            }
        }
...
#pragma omp critical
        {
            if (local_minDistance < minDistance)
            {
                ...
                closestIndex = local_closestIndex;
            }
        }
    }
