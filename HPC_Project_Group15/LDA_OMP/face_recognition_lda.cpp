...
// Change the reduction to use a local accumulator
#pragma omp parallel
    {
        vector<double> local_mean(featureSize, 0.0);
    ...
    int featureSize)
{
    ...
    vector<pair<string, vector<vector<double>>>> samplesVec(classSamples.begin(), classSamples.end());

#pragma omp parallel for
    for (size_t c = 0; c < samplesVec.size(); c++)
    {
        const auto &samples = samplesVec[c].second;
        vector<double> classMean = computeMean(samples);
        vector<vector<double>> centeredSamples = centerData(samples, classMean);

        // Local matrix for each thread to avoid contention
        vector<vector<double>> localSW(featureSize, vector<double>(featureSize, 0.0));
    ...
    int featureSize)
{
    ...
    // Convert map to vector for parallel processing
    vector<pair<string, vector<double>>> meansVec(classMeans.begin(), classMeans.end());

#pragma omp parallel for
    for (size_t c = 0; c < meansVec.size(); c++)
    {
        const auto &classMean = meansVec[c].second;
        int numSamples = classCounts.at(meansVec[c].first);
        ...
        // Local matrix for each thread to avoid contention
        vector<vector<double>> localSB(featureSize, vector<double>(featureSize, 0.0));

        for (int i = 0; i < featureSize; i++)
        {
            for (int j = 0; j < featureSize; j++)
            {
                localSB[i][j] += numSamples * diff[i] * diff[j];
            }
        }
...
// Use parallel section with local accumulator
#pragma omp parallel
    {
        vector<double> local_reconstructed(featureSize, 0.0);
...
#pragma omp for nowait
        for (int i = 0; i < numComponents; i++)
        {
            for (int j = 0; j < featureSize; j++)
            {
                local_reconstructed[j] += projection[i] * projectionMatrix[i][j];
            }
        }
...
#pragma omp critical
        {
            for (int j = 0; j < featureSize; j++)
            {
                reconstructed[j] += local_reconstructed[j];
            }
        }
    }
...
#pragma omp parallel
    {
        double localMinDistance = numeric_limits<double>::max();
...
#pragma omp critical
        {
            if (localMinDistance < minDistance)
            {
                ...
                closestIndex = localClosestIndex;
            }
        }
    }
...
vector<ImageData> loadHierarchicalDataset(const string &rootDir, int &imgWidth, int &imgHeight)
{
    ...
    // Declare atomic variables correctly
    std::atomic<int> imgWidth_atomic{0};
    std::atomic<int> imgHeight_atomic{0};
    ...
    // Resize vector to avoid concurrent pushbacks
    imageData.resize(imagePaths.size());
...
#pragma omp parallel for
    for (size_t i = 0; i < imagePaths.size(); i++)
    {
        ...
        {
            // Atomic operation for first image dimensions
            if (imgWidth_atomic == 0)
            {
                imgWidth_atomic = img.getWidth();
                imgHeight_atomic = img.getHeight();
            }

            if (img.getWidth() == imgWidth_atomic && img.getHeight() == imgHeight_atomic)
            {
                ImageData imgData;
                imgData.filename = path.string();
                imgData.personId = path.parent_path().filename().string();
    ...
    imgWidth = imgWidth_atomic;
    imgHeight = imgHeight_atomic;

    // Remove any empty elements
    imageData.erase(
        remove_if(imageData.begin(), imageData.end(),
                  [](const ImageData &d)
                  { return d.filename.empty(); }),
        imageData.end());
...
// Example in computeCovarianceMatrix:
...
#pragma omp parallel
    {
        vector<vector<double>> local_cov(d, vector<double>(d, 0.0));
